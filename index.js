...
var exts = ["", ".cjs"];
var rules = requireUp("tools/eslint-rules", __dirname);

if (!rules) {
  throw new Error(
    "eslint-plugin-brikl-eslint-local: " +
      'Cannot find "tools/eslint-rules{' +
      [".js"].concat(exts.filter(Boolean)) +
      "} " +
      'or tools/eslint-rules/index.js (checked all ancestors of "' +
      __dirname +
      '").'
  );
}
